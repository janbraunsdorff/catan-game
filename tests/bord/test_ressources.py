...
def test_add_multible_ressource():
    ...
    board = R.add_ressources_to_player(
        board, player, [T.RESSOURCES.Brick, T.RESSOURCES.Wool]
    )
    ...
    assert T.RESSOURCES.Brick in [ressource[x][1]["ressource"] for x in range(2)]
    assert T.RESSOURCES.Wool in [ressource[x][1]["ressource"] for x in range(2)]
...
def test_add_one_ressource_for_two_player():
    ...
    board = R.add_ressource_to_player(board, player_a, T.RESSOURCES.Brick)
    board = R.add_ressource_to_player(board, player_b, T.RESSOURCES.Lumber)
    ...
    assert len(resources) == 1
    assert resources[0] == T.RESSOURCES.Lumber
...
def test_get_ressources_of_player_list_two():
    ...
    assert resources[0] == T.RESSOURCES.Brick
    assert resources[1] == T.RESSOURCES.Brick
...
def test_remove_ressource_from_player():
    ...
    board = R.add_ressource_to_player(board, player, T.RESSOURCES.Brick)
    board = R.remove_ressource_from_player(board, player, T.RESSOURCES.Brick)
